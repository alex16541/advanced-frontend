...
-   `npm run prepare` - Запуск предварительной настройки окружения для проекта (Сейчас - yстанавливает husky)
...
Файлы с переводами хранятся в слайсах компонентов. Так проще работать с переводами и сохранять модульную архитектуру.
...
Для работы с фича тоглами испольуется `featureToggle` и `FeatureToggle` (для компонентов).
